  ...
  "A message serializer" when {
    "not given a manifest" must {
      "handle custom Persistent message serialization" in {
        val persistent = PersistentRepr(MyPayload("a"), 13, "p1", "", true, writerUuid = UUID.randomUUID().toString)
    ...
    "given a PersistentRepr manifest" must {
      "handle custom Persistent message serialization" in {
        val persistent = PersistentRepr(MyPayload("b"), 13, "p1", "", true, writerUuid = UUID.randomUUID().toString)
        val serializer = serialization.findSerializerFor(persistent)
    ...
    "given payload serializer with string manifest" must {
      "handle serialization" in {
        val persistent = PersistentRepr(MyPayload2("a", 17), 13, "p1", "", true, writerUuid = UUID.randomUUID().toString)
        val serializer = serialization.findSerializerFor(persistent)
